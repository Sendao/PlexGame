...
function radCStore( parstr, val ) {
	...
	var bnest = radNest;
	for( i = 0; i < exp.length - 1; ++i ) {
		if( !exp[ i ] in bnest || !supportedObj( bnest[ exp[ i ] ] ) ) {
			radStoreNX( radNest, parstr, val );
			radChange( parstr );
			return;
		}
...
function supportedArray( val ) {
	if( typeof val == 'array' || ( typeof val == 'object' && Object.prototype.toString.call( val ) == '[object Array]' ) )
		return true;
	return false;
}
		...
		for( i = 0; i < exp.length; ++i ) {
			if( i == exp.length - 1 ) {
				...
				} else if( vtype == 0 ) // array
				{
					if( !( exp[ i ] in bnest ) ) {
						bnest[ exp[ i ] ] = [];
					} else if( !supportedArray( bnest[ exp[ i ] ] ) ) {
...
var readable_parms = [ "xWatch", "xWatch2", "xWatch3", "xSect", "xSeed", "xSrc", "xId", "xName",
	...
	"xConnect", "xSuper",
	"xChange", "xOnRender", "xOnSize", "xDraw", "xDisp",
...
var translate_parms = [ "xClass", "xPath", "xOptions", "xCast", "xView", "xAView", "xCmd" ];
var origin_check = [ 'name', 'id', 'src', 'className', 'value' ];
var html_parms = [ 'href', 'rows', 'cols', 'columns', 'size' ];
...
function readDiv( idiv, pdiv ) {
	...
	if( isValid( pdiv ) ) {
		...
		radNode = pdiv = idiv.parentElement;
	} else {
		radNode = idiv;
		pdiv = false;
	}
	...
	if( isValid( x = idiv.origcontent ) ) {
		...
		if( pn ) {
			...
			*/
			...
			console.log("Can't render: " + idiv.origcontent + ": div isolated from dom");
		}
	}
...
//		build.setAttribute( "xroot", xRoot );
	...
	if( isValid( x = idiv.xLoop ) && idiv.style.display != 'none' ) {
		...
		var ixMin = isValid( idiv.xMin ) ? idiv.xMin : 0;
			...
			for( i = idiv.childNodes.length - 1; i >= 0; --i ) {
				...
				} else if( !isValid( xNode.isForZero ) ) // make sure to remember this as a generative node.
				{
					...
					if( xNode.nodeName == "#text" ) {
						newNode = document.createComment( xNode.nodeValue );
...
function injectTemplate( div, tmpname, newroot, seedval ) {
	...
	for( i = 0; i < xn.length; ++i ) {
		...
		/*        if( xn[i].nodeName == '#text' ) {
		          ndiv = document.createTextNode( encodeHTML( xn[i].nodeValue ) );
		          for( var q in xn[i] ) {
		            ndiv[q] = xn[i][q];
		          }
		        } else {*/
